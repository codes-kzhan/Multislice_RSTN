...
class DataLayer(data.Dataset):
	...
	def __getitem__(self, index):
		...
		for i in range(self.slice_thickness / 2):
			index_tmp = self.index1 + i + 1
			if index1 == self.slices - 1:
		...
		last_valid = self.index1
		for i in range(self.slice_thickness - self.slice_thickness/2 - 1):
			index_tmp = self.index1 - i - 1
			if index1 == 0:
				index_tmp = self.index1
			if self.slice_ID[index_tmp] != self.slice_ID[self.index1] - i - 1:
	...
	def load_data(self):
		if self.slice_thickness == 1:
			...
			image[0, ...] = image0
			for i in range(1,self.slice_thickness):

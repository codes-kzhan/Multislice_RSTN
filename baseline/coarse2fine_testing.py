...
for plane in ['X', 'Y', 'Z']:
	...
	for t in range(len(epoch_list)):
		...
		snapshot_state_dict = torch.load(fine_snapshot_[plane][t])
				...
				for plane in ['X', 'Y', 'Z']:
					for t in range(len(epoch_list)):
						...
						for j in range(minR, maxR):
							...
							score_ = score_.reshape(1, slice_thickness, score_.shape[1], score_.shape[2])
							mask_ = mask_.reshape(1, slice_thickness, mask_.shape[1], mask_.shape[2])
							...
							if slice_thickness == 1:
								...
								for i_ID in range(j-int(int(slice_thickness) - int(slice_thickness/2) - 1),j+int(slice_thickness/2)+1):
									...
									pred__[i_ID,:,:] += out[i_ID - (j-int(int(slice_thickness) - int(slice_thickness/2) - 1)), :, :]


						#print('cnt_add:', cnt_add)
						#print('cnt_add == 0:', np.where(cnt_add==0))
						#exit()
						#might have 0 since it uses continue instead change the minR
						if slice_thickness == 3:
							...
							for iii in range(minR, maxR):
								pred__[iii, :, :] /= max(1, cnt_add[iii - minR])
							# for i in range(int(slice_thickness)):
							# 	pred__[minR + i, :, :] /= (i + 1)
							# 	if maxR - i > minR + i:
							# 		pred__[maxR - i - 1, :, :] /= (i + 1)
							# if minR-slice_thickness - 1 >= minR+slice_thickness:
							# 	pred__[minR + slice_thickness :minR-slice_thickness - 1, :, :] /= slice_thickness

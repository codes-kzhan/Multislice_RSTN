...
class FCN8s(nn.Module):
	def __init__(self, n_class=4):
		...
		# conv1
		self.conv1_1 = nn.Conv2d(n_class, 64, 3, padding=100)
	...
	def _initialize_weights(self):
		for name, mod in self.named_children():
			if name == 'saliency1':
				...
				tmp_list = np.arange(1.0,2.000001,1.0/(self.slice_thickness-1))
				mod.bias.data = torch.from_numpy(tmp_list).float()

...
if __name__ == '__main__':
	...
	training_set = DataLayer(data_path=data_path, current_fold=int(current_fold), organ_number=organ_number, \
		...
		organ_ID=organ_ID, plane=plane, filter_slices = False)
	...
	for mode in ['S', 'I', 'J']:
		if mode == 'S':
			...
			# 0. adjust conv1_1 weight
			conv1_1_weight = np.zeros(shape=(64,slice_thickness,3,3))
			conv1_1_weight[:,:(slice_thickness-1),:,:] = pretrained_dict['conv1_1.weight']
			conv1_1_weight[:,(slice_thickness-1),:,:] = np.mean(conv1_1_weight[:,:(slice_thickness-1),:,:],axis=1)
			pretrained_dict['conv1_1.weight'] = torch.from_numpy(conv1_1_weight)
